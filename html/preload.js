...
var GKP = function () {
    ...
    {
        this.identity = 'AGL';
        console.log('Loading as AGL');
        //Without this the user cant use controller mode right after starting the app.
        this.focus    = true;
    }
    else
    {
        this.identity = 'QAS';
        console.log('Loading as QAS');
    }
...
GKP.prototype.onFocus = function () {
    if (this.focus)
    {
        console.warn('onFocus: focus was already true');
    }
    this.focus = true;

    if ('AGL' === this.identity)
    {
        this.windowFocusIn();
    }
...
GKP.prototype._handleGamepadData = function(obj) {
    if (obj.type === 'gamepadEvent' || obj.type === 'psHomeButton') {
        ...
        var windowState = window.gaikai.windowState;
        if (!('QAS' === this.identity && !windowState.aglHasFocus && windowState.allowUnfocusedGamepadInput)
            && (!this.focus || !this._ready))
        {
...
GKP.prototype.requestSwitchGame = function (switchGameSpec) {
    this.sendQASCommand('requestSwitchGame',
                        { 'switchGameSpec': switchGameSpec });
    return this._genPromise();
}
...
GKP.prototype.windowFocusIn = function() {
    this.sendQASCommand('windowFocusIn', {});
}
GKP.prototype.windowFocusOut = function() {
    this.sendQASCommand('windowFocusOut', {});
}
...
GKP.prototype.windowControl = function (command, target) {
    if(command === 'focus')
    {
        if (typeof(target) === 'undefined' || target === this.identity)
        {
            console.error("windowControl focus on itself is not allowed! identity: " + this.identity + " target: " + target);
            return -1;
        }
        else if ('QAS' === target && 'AGL' === this.identity)
        {
            //WINDOWS HACK to call WINAPI AllowSetForegroundWindow from AGL
            this.send({'command' : 'allowSetFGW', params:{} }, 'AGL' );
        }
    }
...
GKP.prototype.sendQASCommand = function(commandName, paramValues) {
    this.send({ command: commandName, params: paramValues }, 'QAS');
}

GKP.prototype.saveDataDeepLink = function(requestId) {
    this.sendQASCommand('saveDataDeepLink',
                        {   'requestId': requestId });
}

GKP.prototype.invitationDeepLink = function(requestId, sessionIdStr, invitationIdStr) {
    this.sendQASCommand('invitationDeepLink',
                        {   'requestId': requestId,
                            'sessionIdStr': sessionIdStr,
                            'invitationIdStr': invitationIdStr
                        });
}

GKP.prototype.gameAlertDeepLink = function(requestId, itemIdStr) {
    this.sendQASCommand('gameAlertDeepLink',
                        {   'requestId': requestId,
                            'itemIdStr': itemIdStr
                        });
}

GKP.prototype.systemStatusDeepLink = function(requestId, hexString) {
    this.sendQASCommand('systemStatusDeepLink',
                        {   'requestId': requestId,
                            'hexString': hexString
                        });
}

GKP.prototype.rawDataDeepLink = function (requestId, bytes) {
    this.sendQASCommand('rawDataDeepLink',
                        {
                            'requestId': requestId,
                            'bytes': bytes
                        });
...
GKP.prototype.audioMute = function()
{
    this.send({ command: 'audioVolumeControl', params: {'command': 'mute'}});
}

GKP.prototype.audioUnmute = function()
{
    this.send({ command: 'audioVolumeControl', params: {'command': 'unmute'}});
}

GKP.prototype.audioSetLevel = function(level) //0-1
{
    this.send({ command: 'audioVolumeControl', params: {'command': 'level', 'level': level}});
}

GKP.prototype.muteMicrophone = function () {
    this.send({ command: 'micControl', params: { 'command': 'mute' } });
}

GKP.prototype.unmuteMicrophone = function () {
    this.send({ command: 'micControl', params: { 'command': 'unmute' } });
}

GKP.prototype.enableUpstreamAudio = function () {
    this.send({ command: 'micControl', params: { 'command': 'enable' } });
}

GKP.prototype.disableUpstreamAudio = function () {
    this.send({ command: 'micControl', params: { 'command': 'disable' } });
}

GKP.prototype.isMicConnected = function () {
    this.send({ command: 'isMicConnected', params: {} }, 'QAS');
}

GKP.prototype.sendMicConnectedEvent = function (connected) {
    this.send({ command: 'sendMicConnectedEvent', params: {'connected': connected}});
}
