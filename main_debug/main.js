...
var argv = minimist(process.argv, {
    ...
    default: {
        config: [__dirname + '/../../../debug.json',  __dirname +  '/debug.json', 'debug.json'],
        devtools: undefined,
        url: undefined,
...
function loadUrl(url)
{
    ...
    if(typeof(mainWindow) !== 'undefined') 
    {
        if (!httpRe.test(url)) 
        {
            ...
            logger.INFO('URL w/o protocol sent (' + oldUrl + ') mapping to', url);
        }
...
function processCLIArgs(app)
{
    ...
    // Set up logging
    if (typeof(argv['debug-level']) !== 'undefined')
    {

        config.debugLevel = parseInt(argv['debug-level']);
        if (isNaN(config.debugLevel)) {
            config.debugLevel = 3;
        } else if (config.debugLevel < 0) {
            config.debugLevel = 0;
        }
    ...
    logger.setLogLevel(config.debugLevel);

    logger.DEBUG(' >>> AGL Startup <<< ');

    if (logLevelChanged) 
    {
        logger.DEBUG('command line debug logging level override: ' + config.debugLevel);
    }
    ...
    {
        config.showDevTools = !(argv.devtools === false);
        logger.DEBUG('command line devtools override: ' + config.showDevTools);
        //
        // logic behind this:
        // with --devtools, then argv.devtools === '', thus should be true
        // with --no-devtools, then argv.devtools === false, thus is false
        // with NO --devtools option, then argv.devtools === undefined,
        //                    thus we never hit this block
    }

    if(typeof(argv['ignore-certificate-errors']) !== 'undefined') {
        config.ignoreCertificateErrors = !(argv['ignore-certificate-errors'] === false);
        logger.DEBUG('command line ignore-certificate-errors override: ' + config.ignoreCertificateErrors);
    }

    if(typeof(argv['start-server']) !== 'undefined') {
        config.startServer = !(argv['start-server'] === false);
        logger.DEBUG('command line start-server override: ' + config.startServer);
    }


    if (typeof(argv['window-frame']) !== 'undefined') {
        config.windowFrame = !(argv['window-frame'] === false);
        logger.DEBUG('command line windowFrame override: ' + argv['window-frame']);
    }

    if (typeof(argv['window-scaling']) !== 'undefined') {
        config.windowScaling = parseFloat(argv['window-scaling']);
        logger.DEBUG('command line windowScaling override: ' + argv['window-scaling']);
        if (config.windowScaling < .05) {
            logger.DEBUG('windowScaling of ' + config.windowScaling + ' is too small, resetting to 0.5');
            config.windowScaling = 0.5;
        }
    }

    if (typeof(argv['content-scaling']) !== 'undefined') {
        config.contentScaling = parseFloat(argv['content-scaling']);
        logger.DEBUG('command line contentScaling override: ' + argv['content-scaling']);
        if (config.contentScaling < .05) {
            logger.DEBUG('contentScaling of ' + config.contentScaling + ' is too small, resetting to 0.5');
            config.contentScaling = 0.5;
        }
    }

    if(!httpRe.test(config.ApolloEndpoint)) {
        config.ApolloEndpoint = 'file://' + __dirname + '/html/' + config.ApolloEndpoint;
    }

    //
    // HAS to be before app.on('ready', ...) triggers
    //
    if(config.ignoreCertificateErrors) {
        app.commandLine.appendSwitch('ignore-certificate-errors');
        logger.WARN("IGNORING CERTIFICATE ERRORS");
    }
